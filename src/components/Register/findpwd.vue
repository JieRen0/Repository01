  ...
  <div class="login-container">
    ...
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <div class="title-container">
        <h3 class="title" style="margin-left:-20%">请修改密码</h3>
      </div>
      ...
      <el-button :loading="loading" id="button1"   @click.native.prevent="handleLogin">修改</el-button>
      ...
      <div class="tips">
        ...
        <span style="margin-left:250px;"><router-link to="/complete">修改成功？去登录！</router-link></span>
      ...
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          ...
          this.$store.dispatch('user/login', this.loginForm).then(() => {
            this.$router.push({ path: this.redirect || './findpwd' })
            this.loading = false
          }).catch(() => {
            this.loading = false
          })
...
.login-container {
  ...
  min-height: 100%;
  width: 100%;
  background: url(../../assets/404_images/findpwd01.png) ;
  background-size:100% ;
  background-color: $bg;
  overflow: hidden;
  display: flex;
